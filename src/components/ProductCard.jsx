    ...
    return product && (
        <div onClick={()=> {navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)}} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full">
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                {/* Producer information display - only for producer-posted products */}
                {product.producerId && (
                    <div className="flex items-center gap-2 mt-2 mb-1">
                        <img 
                            src={product.producerId.profileImageUrl || assets.profile_icon} 
                            alt={product.producerId.name}
                            className="w-6 h-6 rounded-full object-cover border border-gray-200"
                        />
                        <p className="text-xs text-gray-600">By {product.producerId.name}</p>
                    </div>
                )}
